...
def main():
    ...
    # --- UNCパスから直接起動されていないか確認 ---
    if is_running_from_unc():
        root = tk.Tk()
        root.withdraw()
        mb.showerror(
            "起動エラー",
            "このexeをネットワーク共有フォルダから直接起動することはできません。\n\n"
            "ローカルPC（デスクトップ等）にコピーしてから起動してください。\n\n"
            "【手順】\n"
            "1. app\\current\\Koudouhyo.exe をデスクトップ等にコピー\n"
            "2. config.json を同じフォルダにコピー\n"
            "3. ローカルのexeを起動"
        )
        sys.exit(1)
        ...
        if answer:
            try:
                ...
                mb.showinfo(
                    ...
                    f"1. 以下のフォルダを開く\n"
                    f"   {app_settings.shared_root}\\app\\current\\\n"
                    f"2. Koudouhyo.exe と config.json を\n"
                    f"   デスクトップ等にコピー\n"
                    f"3. ローカルのexeを起動する"
                )
                root2.destroy()
